...
| <a name="input_create_networking_prereqs"></a> [create\_networking\_prereqs](#input\_create\_networking\_prereqs) | Create networking resources required for ACI to be deployed. | `bool` | `true` | no |
...
| <a name="input_mssql_config"></a> [mssql\_config](#input\_mssql\_config) | MSSQL configuration object to create persistent SQL server instance for sonarqube aci. | <pre>object({<br>    name    = string<br>    version = string<br>  })</pre> | <pre>{<br>  "name": "sonarqubemssql6284",<br>  "version": "12.0"<br>}</pre> | no |
