...
Ensure that any clients accessing the SonarQube instance have the relevant network peering in place with the SonarQube VNET and that the Private DNS custom local domain `custom.local` is linked with relevant VNETs for domain name solution of teh sonarqube instance.  
